...
def visualize_constraint_graph(
    ...
    try:
        ...
        # Get graph structure from Python dictionaries
        # ConstraintGraph stores nodes and edges in .nodes and .edges dicts
        ...
        edge_labels = {}
        for edge in edges:
            # Get edge connection info via CORBA
            try:
                edge_info = graph.getNodesConnectedByEdge(edge)
                from_node = edge_info[0]
                to_node = edge_info[1]
                G.add_edge(from_node, to_node)
                edge_labels[(from_node, to_node)] = edge
            except Exception as e:
                print(f"  ⚠ Could not get info for edge '{edge}': {e}")
