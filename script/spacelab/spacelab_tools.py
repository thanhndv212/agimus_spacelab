...
class GraphBuilder:
    ...
    def __init__(self, planner, robot, ps, backend: str = "corba"):
        ...
        Args:
            planner: Planner instance (CorbaManipulationPlanner or PyHPPManipulationPlanner)
    ...
    def create_manual_graph(self, name: str = "graph") -> Any:
        ...
        if self.backend == "corba":
            ...
            try:
                ...
                raise ImportError("PyHPP manipulation module not available")
    ...
    def add_state(self, name: str, is_waypoint: bool = False, 
        ...
        if self.graph is None:
            raise RuntimeError("Graph not initialized. Call create_manual_graph() first")
    ...
    def add_edge(self, from_state: str, to_state: str, name: str,
                 weight: int = 1, containing_state: Optional[str] = None) -> int:
        """
        ...
        self.edge_topology[name] = (from_state, to_state)
        print(f"    ✓ Edge '{name}': {from_state} → {to_state} (ID: {edge_id})")
        return edge_id
    ...
    def finalize_manual_graph(self) -> Any:
        ...
        print(f"   Finalizing graph with {len(self.states)} states and {len(self.edges)} edges")
    ...
    def create_factory_graph(
        ...
        """
        if self.backend != "corba":
            raise NotImplementedError("Factory mode currently only supported for CORBA backend")
        
        if not HAS_CORBA:
            raise ImportError("CORBA backend not available")
        
        print(f"   Creating factory-based constraint graph")
        print(f"     Grippers: {grippers}")
        print(f"     Objects: {objects}")
        
        from hpp.corbaserver.manipulation import ConstraintGraph, ConstraintGraphFactory, Rule
        ...
        try:
            ...
            for edge_name in self.graph.edges.keys():
                ...
                # Try to get topology
                try:
                    from_node, to_node = self.graph.getNodesConnectedByEdge(edge_name)[:2]
        ...
        print(f"   ✓ Factory graph created with {len(self.states)} states and {len(self.edges)} edges")
        return self.graph
    ...
    def apply_state_constraints(
        ...
        if self.backend == "corba":
            ...
            # Apply constraints
            success, q_proj, error = self.graph.applyConstraints(state_name, list(q))
    ...
    def get_edge_topology(self) -> Dict[str, Tuple[str, str]]:
        """Get dictionary of edge names to (from_state, to_state) tuples."""
