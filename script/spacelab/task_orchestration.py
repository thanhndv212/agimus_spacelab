...
- Behavior tree integration for dynamic task composition
- Synchronization points for collaborative actions

Architecture:
    AtomicTask -> TaskComposer -> BehaviorTree -> Orchestrator
"""
...
class AtomicTask:
    ...
    An atomic task represents a single manipulation action that cannot be
    further decomposed (e.g., "grasp object", "place object", "handover").
    """
    task_id: str
    name: str
    description: str
    ...
    enables: List[str] = field(default_factory=list)  # Task IDs enabled after completion
    ...
    def can_execute(self, resource_manager: 'ResourceManager') -> Tuple[bool, str]:
        """
...
class ResourceManager:
    """
    Manages allocation and deallocation of resources (arms, objects, workspace).
    
    Ensures no conflicts in resource usage across concurrent tasks.
    """
    ...
    def allocate(self, task_id: str, resources: Set[Resource]) -> bool:
        ...
        Returns:
            True if all resources allocated successfully
        """
    ...
    def get_allocation_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "allocated": sum(1 for r in self.resources.values() if not r.available),
            "allocations_by_task": {
                task_id: [f"{r.resource_type.value}:{r.name}" for r in resources]
...
class TaskDependencyGraph:
    ...
    - Concurrent execution (A and B can run in parallel)
    - Conditional dependencies (B needs A only if condition X)
    """
    
    def __init__(self):
        self.tasks: Dict[str, AtomicTask] = {}
        self.dependencies: Dict[str, Set[str]] = {}  # task_id -> prerequisite task_ids
    ...
    def get_concurrent_tasks(self, ready_tasks: List[AtomicTask],
                           resource_manager: ResourceManager) -> List[AtomicTask]:
        """
    ...
    def has_circular_dependency(self) -> bool:
        """Check for circular dependencies (would cause deadlock)."""
...
class TaskOrchestrator:
    ...
    def setup_resources(self, arms: List[str], objects: List[str],
    ...
    def can_execute_task(self, task: AtomicTask) -> Tuple[bool, str]:
        """Check if task can be executed now."""
        # Check preconditions and resources
    ...
    def execute_task(self, task: AtomicTask) -> bool:
        ...
        # Allocate resources
        if not self.resource_manager.allocate(task.task_id, task.required_resources):
        ...
        try:
            ...
            if success:
                ...
                if verified:
                    ...
                    print(f"  ✓ {task.name} completed ({task.get_duration():.2f}s)")
                else:
    ...
    def run(self, dry_run: bool = False) -> bool:
        ...
        iteration = 0
        max_iterations = len(self.dependency_graph.tasks) * 10  # Prevent infinite loop
        ...
        while iteration < max_iterations:
            ...
            # Get concurrent tasks (considering resources)
...
class TaskBuilder:
    ...
    def requires_object(self, obj_name: str) -> 'TaskBuilder':
        self.task.required_resources.add(Resource(ResourceType.OBJECT, obj_name))
        return self
        
    def requires_workspace(self, ws_name: str) -> 'TaskBuilder':
        self.task.required_resources.add(Resource(ResourceType.WORKSPACE, ws_name))
        return self
    ...
    def with_precondition(self, desc: str, check: Callable[[], bool]) -> 'TaskBuilder':
        self.task.preconditions.append(TaskPrecondition(desc, check))
        return self
        
    def with_postcondition(self, desc: str, verify: Callable[[], bool]) -> 'TaskBuilder':
